...
import { EyeIcon, PencilIcon, PlusCircleIcon, TrashIcon, ArrowDownCircleIcon } from '@heroicons/vue/24/outline';

import { Driver, paginated } from '../../types/interface';
...
const handleDelete = () => {
    ...
    deleteForm.delete(route('users.drivers.delete', {id: targetToDelete.value}),{
        onSuccess: ()=>{
                    ...
                    <div class="p-6 text-gray-900">
                        <div class="overflow-x-auto">
                            <table class="table">
                                ...
                                <tbody>
                                    <tr v-for="(driver, index) in props.drivers.data">
                                        <td>{{ index+1 }}</td>
                                        <td>{{ driver.firstname}} {{ driver.lastname}}</td>
                                        <td>{{ driver.email}}</td>
                                        <td>{{ driver.barangay }}</td>
                                        <td>{{driver.assigned_truck.plate_number}}</td>
                                        <td>
                                            <div class="flex gap-2">
                                                <Link class="inline-flex items-center btn px-4 py-3 text-white bg-green-600 dark:bg-gray-800 border border-gray-300 dark:border-gray-500 rounded-md font-semibold text-xs dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-green-400 dark:hover:bg-gray-700 focus:outline-none disabled:opacity-25 transition ease-in-out duration-150" :href="route('users.drivers.edit', {id: driver.id})"><PencilIcon class="h-4" /></Link>
                                                <LinkButton class="!bg-blue-600" :href="route('users.drivers.show', {id: driver.id})" label="">
                                                    <template #icon>
                                                        <EyeIcon class="h-4" />
                                                    </template>
                                                </LinkButton>
                                                <SecondaryButton @click="targetToDelete = driver.id" onclick="deleteModal.showModal()" class="!bg-red-600 text-white"> <TrashIcon class="h-4" /> </SecondaryButton>
                                            </div>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                            <Pagination :paginate="props.drivers" />
                        </div>
                    </div>
                </div>
            </div>
