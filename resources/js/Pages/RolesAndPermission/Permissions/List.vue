...
const handleDelete = () => {
    ...
    deleteForm.delete(route('users.permissions.delete', {id: targetToDelete.value}), {
        onSuccess: ()=>{
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 ">
                <Link :href="route('users.permissions.create')" class="disabled:text-gray-500 inline-flex hover:border-transparent items-center px-4 py-3 btn bg-slate-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-kwikweb dark:hover:bg-white focus:bg-kwikweb-200 dark:focus:bg-white active:bg-kwikweb-900 dark:active:bg-gray-300 focus:outline-none focus:ring-offset-2 transition ease-in-out duration-150"><PlusCircleIcon class="h-7" />Add Permission</Link>
                    ...
                    <div class="p-6 text-gray-900">
                        <div class="overflow-x-auto">
                            <table class="table">
                                ...
                                <tbody>
                                    <tr v-for="(permission, index) in props.permissions">
                                        <td>{{ index+1 }}</td>
                                        <td>{{ permission.name}}</td>
                                        <td></td>
                                        <td>
                                            <div class="flex gap-2">
                                                <Link class="inline-flex items-center btn px-4 py-3 text-white bg-green-600 dark:bg-gray-800 border border-gray-300 dark:border-gray-500 rounded-md font-semibold text-xs dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-green-400 dark:hover:bg-gray-700 focus:outline-none disabled:opacity-25 transition ease-in-out duration-150" :href="route('users.permissions.edit', {id: permission.id})"><PencilIcon class="h-4" /></Link>
                                                <SecondaryButton @click="targetToDelete = permission.id" onclick="deleteModal.showModal()"  class="!bg-red-600 text-white"> <TrashIcon class="h-4" /> </SecondaryButton>
                                            </div>
                                        </td>
                                    </tr>
                                </tbody>
        ...
        <Modal id="deleteModal" title="Permission Delete Form">
            <template #body>
                <div class="mt-5">
                    <span class="text-red-700">You are about to delete this permission?</span>
                </div>
            </template>
            <template #actions>
